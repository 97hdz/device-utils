...
Device.isIphone13Pro = function isIphone13Pro(force){
	if ( !force && _.isBoolean(deviceCache.isIphone13Pro) )
		return deviceCache.isIphone13Pro;
	deviceCache.isIphone13Pro = Device.isIphone13(force);
	return deviceCache.isIphone13Pro;
}

Device.isIphone14Pro = function isIphone14Pro(force){
	if ( !force && _.isBoolean(deviceCache.isIphone14Pro) )
		return deviceCache.isIphone14Pro;

	var iOs = Device.isIos(force);
	if (!iOs){
		deviceCache.isIphone14Pro = false;
		return false;
	}

	var dpr = _.isNumber(devicePixelRatio) && !_.isNaN(devicePixelRatio) ? devicePixelRatio : 1;
	deviceCache.isIphone14Pro = screen.height * dpr == 2556; // @ref: https://developer.apple.com/ios/human-interface-guidelines/visual-design/adaptivity-and-layout/
...
Device.isIphone12ProMax = function isIphone12ProMax(force){
	...
	Device.isIphone13ProMax = function isIphone13ProMax(force) {
		if (!force && _.isBoolean(deviceCache.isIphone13ProMax))
			return deviceCache.isIphone13ProMax;

		var iOs = Device.isIos(force);
		if (!iOs) {
			deviceCache.isIphone13ProMax = false;
			return false;
		}

		var dpr = _.isNumber(devicePixelRatio) && !_.isNaN(devicePixelRatio) ? devicePixelRatio : 1;
		deviceCache.isIphone13ProMax = screen.height * dpr == 2778; // @ref: https://developer.apple.com/ios/human-interface-guidelines/visual-design/adaptivity-and-layout/

		return deviceCache.isIphone13ProMax;
	}

	Device.isIphone14ProMax = function isIphone14ProMax(force) {
		if (!force && _.isBoolean(deviceCache.isIphone14ProMax))
			return deviceCache.isIphone14ProMax;

		var iOs = Device.isIos(force);
		if (!iOs) {
			deviceCache.isIphone14ProMax = false;
			return false;
		}

		var dpr = _.isNumber(devicePixelRatio) && !_.isNaN(devicePixelRatio) ? devicePixelRatio : 1;
		deviceCache.isIphone14ProMax = screen.height * dpr == 2778; // @ref: https://developer.apple.com/ios/human-interface-guidelines/visual-design/adaptivity-and-layout/
...
Device.isIphoneWithNotch = function isIphoneWithNotch(force) {
	return Device.isIphoneX(force)
		...
		|| Device.isIphone13Pro(force)
		|| Device.isIphone14Pro(force)
		|| Device.isIphone12ProMax(force)
		|| Device.isIphone13ProMax(force)
		|| Device.isIphone14ProMax(force);
}
